...
def download_pdf_receipt(v, lab_phone):
    ...
    pdf.cell(0, 5, f"Date: {str(v[2])}", align='R', ln=True)
    pdf.cell(30, 5, f"Name: {str(v[3])}")
    pdf.cell(0, 5, f"Age/Sex: {str(v[5])}/{str(v[6])}", align='R', ln=True)
    pdf.ln(2)
    pdf.cell(0, 5, f"Ref By: {str(v[7])}", ln=True)
    pdf.ln(2)
