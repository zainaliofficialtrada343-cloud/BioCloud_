...
def show_receipt(v):
    """
    v is the list/row of patient data.
    Order: [ID, Invoice, Date, Name, Mobile, Age, Gender, Collected, Test, Total_Bill, Paid_Amount, Remaining, Result, Unit, Status]
    """
    try:
        # HTML aur CSS ko yahan alag rakha hai taake app.py saaf rahe
        receipt_html = f"""
        <style>
            @media print {{
        ...
        receipt_html += f"""
            ...
            <p style="text-align: center; font-size: 9px; margin-top: 20px;">
                Developed by Zain - 03702906075<br>*** Lab Management System ***
            </p>
        </div>
        """
